...
def setTime():
    ...
    # Calculate green time formula
    greenTime = 10#math.ceil(((noOfCars*carTime) + (noOfRickshaws*rickshawTime) + (noOfBuses*busTime) + (noOfTrucks*truckTime)+ (noOfBikes*bikeTime))/(noOfLanes+1))
...
def simulationTime():
    ...
    while(True):
        ...
        if(timeElapsed==simTime):
            ...
            print('No. of vehicles passed per unit time: BaseSimi :  ',(float(totalVehicles)/float(timeElapsed)))
            os._exit(1)
...
def Main():
    ...
    pygame.display.set_caption("Base simulation")
